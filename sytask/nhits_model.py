...
from neuralforecast.core import NeuralForecast      # pip install neuralforecast, pip install scipy
...
class NHITSModel:
    ...
    def read_csv(self):
        # CSV 파일 경로 지정
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config', 'train_data.csv')
        # CSV 파일 읽기
    ...
    def reasoning_model(self):
        ...
        input_data = joblib.load('C:/CNU_Program/network/2025_Network_AI_hackathon/sytask/config/test_inputs_sample.pkl')
        output_data = joblib.load('C:/CNU_Program/network/2025_Network_AI_hackathon/sytask/config/test_outputs_sample.pkl')
        print(input_data[:, :, 6][163000])
        print(output_data[:, :][0])
        future_ds = pd.date_range('2025-01-01', periods=100, freq='100ms')
        # 3. 불러온 array 길이와 future_ds 길이 확인 (같아야 함)
        assert len(input_data[:, :, 6][0]) == len(future_ds), "y array length and future_ds length must match"
        n = 100
        test_df = pd.DataFrame({
            'unique_id': ['test_series'] * n,
            'ds': pd.date_range('2025-01-01', periods=n, freq='100ms'),
            'y': input_data[:, :, 6][0]  # 예측하려는 과거 100개 값 (1D array or list)
        })
         # 4. 예측 수행 (최근 시점부터 향후 10개 시점 예측)
        ...
        # 1. 과거 데이터(실측) 부분: 마지막 INPUT_SIZE 구간만 표시
        history_df = test_df
        plt.figure(figsize=(12, 5))
        plt.plot(history_df['ds'], history_df['y'], label='실측값 (History)', color='blue')

        # 2. 예측 결과: forecasts 데이터프레임 사용
        plt.plot(forecasts['ds'], forecasts['NHITS'], label='예측값 (Forecast, NHITS)', color='red', marker='o')

        # 3. 그래프 옵션
        plt.xlabel('시점')
        plt.ylabel('traffic_volume')
        plt.title('실측값과 예측값(NHITS) 시계열')
        plt.legend()
        plt.tight_layout()
        plt.show()
    ...
    def save_parameter(self, nf):
        nf.save(
            path=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model'),   # 모델 저장 경로
    ...
    def load_parameter(self):
        nf = NeuralForecast.load(path=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model'))
        return nf
